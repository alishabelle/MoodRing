$(function () {
  var primaryColor = "#000000",
    ...
    blendedColor = "#000000";
  let canvas = document.getElementById('picker-colors');
  let context = canvas.getContext("2d");
  let canvasHelper = document.getElementById("picker-container");
  let contextHelper = canvasHelper.getContext("2d");
  ...
  function dynamicCircle() {
    ...
    size = $(".item-picker").width();
    $(".item-picker").height(size);
    canvas.width = size;
    canvas.height = size;
    canvasHelper.width = size;
    canvasHelper.height = size;
  ...
  function drawCircle() {
    ...
    for (let x = -radius; x < radius; x++) {
      for (let y = -radius; y < radius; y++) {
        ...
        if (r > radius || r < radius / 2.25) {
          // skip all (x,y) coordinates that are outside of the circle
          // also cut a hole in the center to create a ring shape
          continue;
        }
    ...
    let containerRadius = canvasHelper.width / 2;

    //outer white border
    contextHelper.beginPath();
    contextHelper.strokeStyle = "white";
    contextHelper.lineWidth = 5;
    contextHelper.arc(canvasHelper.width / 2, canvasHelper.height / 2, containerRadius - contextHelper.lineWidth / 2, 0, 2 * Math.PI);
    contextHelper.stroke();

    //inner white border
    contextHelper.beginPath();
    contextHelper.arc(canvasHelper.width / 2, canvasHelper.height / 2, containerRadius / 2.25, 0, 2 * Math.PI);
    contextHelper.stroke();
  }
  ...
  function initColorNames(namesArray) {
    ...
    for (var i = 0; i < namesArray.length; i++) {
      ...
      namesArray[i].push(rgb[0], rgb[1], rgb[2], hsl[0], hsl[1], hsl[2]);
    }
  }
  ...
  $("#action-button").on("click", function (event) {
    if (primaryColor !== "#000000" && secondaryColor !== "#000000") {
      //blend colors once there is a selection
      ...
      $("#picker-colors").addClass("rotating").one('animationend webkitAnimationEnd oAnimationEnd', function () {
            $("#picker-colors").removeClass("rotating");
...
//  let sum = parseInt(color1.slice(1), 16) + parseInt(color2.slice(1), 16);
//  let quotient = (sum / 2).toString(16);
//  return '#' + '0'.repeat(6 - quotient.length) + quotient;
//}



// converts from HSV to RGB
// hue assumed in range [0, 360]
// saturation, value assumed in range [0,1]
// return [r,g,b] each in range [0,255]
// Equation src: https://en.wikipedia.org/wiki/HSL_and_HSV#From_HSV
